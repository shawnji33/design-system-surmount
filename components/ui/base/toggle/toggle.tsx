...
export const ToggleBase = ({ className, isHovered, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    ...
    return (
        <div
            className={cx(
                "cursor-pointer rounded-full bg-tertiary ring-[0.5px] ring-border-secondary outline-focus-ring transition duration-150 ease-linear ring-inset",
                isSelected && "bg-fg-primary",
                isSelected && isHovered && "bg-fg-primary",
                ...
                <>
                    ...
                    {(label || hint) && (
                        <div className={cx("flex flex-col", styles[size].textWrapper)}>
                            ...
                            {hint && (
                                <span className={cx("text-fg-tertiary", styles[size].hint)} onClick={(event) => event.stopPropagation()}>
                                    {hint}
                                </span>
                            )}
                        </div>
